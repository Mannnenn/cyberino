...
PID posturePD(50.0f, 0.0f, 0.025f, 0.0f, -30.0f, 30.0f);   // 姿勢制御用のPDコントローラー（目標角度 - 実測角度）用，直接IMUの角度微分をD項に使用するため不完全微分は不要、出力は速度指令なので±30 rad/s程度を想定
...
void setup()
{
  ...
  delay(100);
  cybergear->set_limit_speed(1, 30.0);
  delay(100);
...
void loop()
{
  ...
  if (currentTime - lastSpeedUpdateTime >= SPEED_UPDATE_INTERVAL_US)
  {
    ...
    if (motorEnabled)
    {
      ...
      cybergear->set_speed(1, target_speed);
    }
